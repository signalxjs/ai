/**
 * @sigx/ai-ui/web — the base catalog on the web: eight plain functions over
...
export const webComponents: Record<string, (p: UIComponentProps) => JSXElement> = {
    stack: (p) => jsx('div', { class: classes('ai-ui-stack', p.class), style: flex(p), children: p.children }),
    ...
    text: (p) => {
        ...
        return jsx(TEXT_TAG[variant] ?? 'span', { class: classes('ai-ui-text', `ai-ui-text--${variant}`, p.class), style, ...press(p), children: str(p.text) });
    },

    button: (p) =>
        jsx('button', {
            type: 'button',
            class: classes('ai-ui-button', `ai-ui-button--${str(p.variant, 'primary')}`, p.size ? `ai-ui-button--${str(p.size)}` : '', p.pending ? 'is-pending' : '', p.class),
    ...
    input: (p) => {
        const field = jsx('input', {
            class: classes('ai-ui-input', p.class),
            style: styleOf(p.style),
        ...
        if (typeof p.label === 'string' && p.label) {
            return jsx('label', { class: 'ai-ui-field', children: [jsx('span', { class: 'ai-ui-field__label', children: p.label }), field] });
        }
        return field;
    },
    ...
    image: (p) =>
        jsx('img', {
            class: classes('ai-ui-image', p.class),
            src: str(p.src),
            alt: str(p.alt),
    ...
    list: (p) => jsx('div', { class: classes('ai-ui-list', p.class), style: flex(p), children: p.children }),

    card: (p) => {
        const style: Style = { ...(typeof p.padding === 'number' ? { padding: `${p.padding}px` } : {}), ...styleOf(p.style) };
        const children: JSXElement[] = [];
        if (typeof p.title === 'string' && p.title) children.push(jsx('div', { class: 'ai-ui-card__title', children: p.title }));
        children.push(jsx('div', { class: 'ai-ui-card__body', children: p.children }));
        return jsx('div', { class: classes('ai-ui-card', p.class), style, children });
    },

    divider: (p) => jsx('hr', { class: classes('ai-ui-divider', p.class), style: styleOf(p.style) })
};
...
.ai-ui-stack, .ai-ui-list { min-width: 0; }
.ai-ui-text { display: block; margin: 0; line-height: 1.4; }
.ai-ui-text--heading { font-size: 1.35rem; font-weight: 650; }
.ai-ui-text--title { font-size: 1.1rem; font-weight: 600; }
.ai-ui-text--caption { font-size: 0.85em; opacity: 0.7; }
.ai-ui-text--label { font-size: 0.85em; font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; opacity: 0.75; }
.ai-ui-button { font: inherit; border-radius: 8px; padding: 0.45em 0.9em; border: 1px solid transparent; cursor: pointer; }
.ai-ui-button--primary { background: #2563eb; color: #fff; }
.ai-ui-button--secondary { background: #e5e7eb; color: #111; }
.ai-ui-button--ghost { background: transparent; border-color: currentColor; }
.ai-ui-button--danger { background: #dc2626; color: #fff; }
.ai-ui-button--sm { font-size: 0.85em; padding: 0.3em 0.7em; }
.ai-ui-button--lg { font-size: 1.1em; padding: 0.6em 1.2em; }
.ai-ui-button:disabled { opacity: 0.55; cursor: default; }
.ai-ui-input { font: inherit; padding: 0.45em 0.6em; border: 1px solid #cbd5e1; border-radius: 8px; width: 100%; box-sizing: border-box; }
.ai-ui-field { display: flex; flex-direction: column; gap: 4px; }
.ai-ui-field__label { font-size: 0.85em; opacity: 0.8; }
.ai-ui-card { border: 1px solid #e2e8f0; border-radius: 12px; padding: 12px; background: #fff; }
.ai-ui-card__title { font-weight: 600; margin-bottom: 8px; }
.ai-ui-card__body { display: flex; flex-direction: column; gap: 8px; }
.ai-ui-divider { border: 0; border-top: 1px solid #e2e8f0; margin: 4px 0; width: 100%; }
.ai-ui-image { max-width: 100%; border-radius: 8px; }
`;
