...
export function describeCatalog(catalog: UICatalog, options: DescribeOptions = {}): string {
    ...
    out.push('Steps run in order. Consecutive steps that carry "if" form ONE decision, like a switch: all their conditions are judged against the state as it was before the first of them ran, so the cases never see each other\'s writes (guards "overwrite" and "!overwrite" on adjacent steps are safe). A step without "if" ends the group; it and anything after it see the writes. Values are always resolved live, right before the step.');
