...
UI from JSON for [SignalX](https://sigx.dev/) — a JSON UI spec that
anything can write and stream (a model, a server, a CMS, a low-code
editor), the catalog that says what may be built and validates what was,
a safe expression language, reactive state, pluggable async actions, and a
renderer on `@sigx/runtime-core` that works wherever sigx renders. No
dependency on `@sigx/ai`; `@sigx/ai/ui` is the thin bridge that hands a
catalog to a model as a tool. **Proof of concept** — the shape is settling.

Three entries:

| Entry | What |
|---|---|
| `@sigx/json-ui` | the spec, expressions, catalog + validation, the stream reducer, the action runtime |
| `@sigx/json-ui/app` | `UIView` and `createUIRuntime` — the renderer, on `@sigx/runtime-core` (never the `sigx` umbrella) |
| `@sigx/json-ui/web` | `webRegistry()` — the base catalog as HTML elements, plus `webStyles` |

## With a model: the whole thing in one chat turn

```ts
// server — the model gets a tool whose description IS the catalog (`@sigx/ai/ui`)
...
import { UIView } from '@sigx/json-ui/app';
import { webRegistry } from '@sigx/json-ui/web';
...
follow) and `describeCatalog(catalog)` (a plain-text description of the
catalog, written for a model but usable as docs). `uiTool()` on
`@sigx/ai/ui` bundles them into a tool.
